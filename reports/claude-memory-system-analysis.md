...
2. [Memory 优先级体系](#memory-优先级体系)
3. [CLAUDE.md 文件加载机制](#claudemd-文件加载机制)
4. [Rules 懒加载机制](#rules-懒加载机制)
5. [Agent Memory 持久记忆](#agent-memory-持久记忆)
6. [Auto Memory 自动记忆](#auto-memory-自动记忆)
7. [四层记忆体系对比](#四层记忆体系对比)
8. [完整执行流程](#完整执行流程)
9. [本仓库实现示例](#本仓库实现示例)
10. [最佳实践](#最佳实践)
...
当多个 CLAUDE.md 文件同时加载时，**越具体（越靠近当前目录）的文件优先级越高**：

```
优先级（高 → 低）

1. CLAUDE.local.md（项目本地，git 忽略）← 最高优先级
   ↓ 覆盖
2. .claude/CLAUDE.md（项目私有目录）
   ↓ 覆盖
3. CLAUDE.md（项目根目录，团队共享）
   ↓ 覆盖
4. 子目录 CLAUDE.md（懒加载，路径相关）
   ↓ 覆盖
5. ~/.claude/CLAUDE.md（全局个人默认）← 最低优先级
```

**重要**：CLAUDE.md 文件通常是**累加的**（所有加载的文件内容都注入上下文），而非严格替换。当指令冲突时，Claude 遵循更具体的指令。

### 2.2 完整优先级对照表

| 优先级 | 文件 | 作用域 | 版本控制 | 共享 | 说明 |
|--------|------|--------|---------|------|------|
| **1（最高）** | `CLAUDE.local.md` | 项目 | ❌ git 忽略 | ❌ 个人 | 个人项目覆盖 |
| **2** | `.claude/CLAUDE.md` | 项目 | ✅ 可选 | ✅ 可选 | 项目私有目录 |
| **3** | `CLAUDE.md`（项目根） | 项目 | ✅ 是 | ✅ 是 | 团队共享规范 |
| **4** | 子目录 `CLAUDE.md` | 子目录 | ✅ 是 | ✅ 是 | 懒加载，路径相关 |
| **5（最低）** | `~/.claude/CLAUDE.md` | 全局 | ❌ 否 | ❌ 个人 | 所有项目默认 |

### 2.3 Rules 在优先级中的位置

`.claude/rules/` 不参与上述优先级链，它是**补充性的条件加载**：
...
Rules 与 CLAUDE.md 是**并列关系**，不是覆盖关系。当 Rules 与 CLAUDE.md 冲突时，Claude 根据具体性和上下文判断。

### 2.4 Agent Memory 作用域优先级

Agent Memory 的三个作用域也遵循"越具体越优先"原则：

```
优先级（高 → 低）

1. local（.claude/agent-memory-local/<name>/）← 最高
   ↓ 覆盖
2. project（.claude/agent-memory/<name>/）
   ↓ 覆盖
3. user（~/.claude/agent-memory/<name>/）← 最低
```

**注意**：Agent 只能配置一个 `memory` 作用域，三者不会同时加载。优先级体现在**选择哪个作用域**时的决策逻辑。

### 2.5 与 Settings 优先级的对应关系

Memory 优先级体系与 Settings 配置层级完全对应：

| Settings 层级 | 对应 Memory 文件 | 优先级 |
|--------------|----------------|--------|
| Managed（组织强制） | — | 超越所有 |
| CLI 参数 | — | 会话级 |
| `.claude/settings.local.json` | `CLAUDE.local.md` | 最高（用户可写） |
| `.claude/settings.json` | `CLAUDE.md`（项目根） | 团队共享 |
| `~/.claude/settings.json` | `~/.claude/CLAUDE.md` | 全局默认 |

这种一致性设计让整个 Claude Code 配置体系形成统一的心智模型：**本地 > 项目 > 全局**。
